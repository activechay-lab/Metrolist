...
fun Song.toMediaItem(allowBlacklistedPlayback: Boolean = false): MediaItem {
    val mediaItem = toMediaMetadata().toMediaItem()
    if (!allowBlacklistedPlayback) return mediaItem
    // Preserve upstream's extras (artwork_uri) rather than replacing them --
    // this only adds the blacklist-playback-override flag MusicService reads
    // back at playback time (see skipIfBlacklisted).
    return mediaItem.buildUpon()
        .setMediaMetadata(
            mediaItem.mediaMetadata.buildUpon()
                .setExtras(Bundle(mediaItem.mediaMetadata.extras ?: Bundle()).apply {
